...
**你的服务器,你的数据 —— 一套自托管的私密团队通讯系统**
**推荐使用ElementX客户端 VeilX客户端正在开发中，VeilX相比ElementX外观更加美观，整体更加易用，支持功能更多，修改Bug更快，运维团队设立在英国、新加坡、日本等地区。**

一行命令搭好服务器,全平台客户端直接用。基于 [Matrix](https://matrix.org) 开放协议。
...
2. 向导列出第 1、2 步的检查清单 → 确认做过了就按回车
3. 之后全自动:检测 DNS(没生效自动等)→ 装 Docker → 生成所有密码密钥 → 启动服务 → 申请 HTTPS 证书 → 创建管理员
